---
name: crosscheck
description: Get an independent second opinion on a draft before your human sees it. Use when you have written an email, message, report, summary, PR description, or plan for your human and want it checked for wrong facts or arithmetic, contradictions, leftover placeholders, leaked secrets, unauthorized commitments, or tone. Paid per check ($0.02 USDC on Base) over x402; returns a JSON verdict and a signed receipt.
license: MIT
...
Use your own agent's skills directory in place of `~/.claude/skills`. The two scripts are self-contained bundles that need only Node 20 or later. Their SHA-256 checksums are in `SHA256SUMS` and at https://crosscheckapi.com/skill/SHA256SUMS.
...
1. **MCP server** with tools `quote`, `order`, and `result`. From npm (`crosscheckapi`, listed in the MCP Registry as `com.crosscheckapi/crosscheck`):
   ```json
